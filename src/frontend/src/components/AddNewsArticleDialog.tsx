...
import { useAddNewsArticle } from '../hooks/useAddNewsArticle';
import { ExternalBlob, NewsCategory, ArticleType } from '../backend';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function AddNewsArticleDialog({ open, onOpenChange, onSuccess }: AddNewsArticleDialogProps) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await addArticle.mutateAsync(articleInput);
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <Select onValueChange={(value) => setValue('category', value as NewsCategory)}>
              <SelectTrigger>
          ...
          <div>
            ...
            />
            {errors.featuredImage && <p className="text-sm text-bold-red mt-1">{errors.featuredImage.message}</p>}
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {addArticle.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Add Article
            </Button>
